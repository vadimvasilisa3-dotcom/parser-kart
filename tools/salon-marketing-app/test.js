...
// 10. HTML: все контейнеры, на которые ссылается app.js
{
  ...
  ["cl-yandexMaps", "cl-gis", "cl-reviews", "int-list", "diag-result", "smm-plan", "ads-forecast", "an-result",
    ...
    "connect-modal", "card-vk", "card-yandex"].forEach((id) => {
  ...
  check("v2: connect modal в app.js", app.includes("openConnectModal"));
  check("v2: oauth-bridge.html", fs.existsSync(__dirname + "/oauth-bridge.html"));
  ...
  check("Онбординг: панель howto вне профиля", html.includes('id="howto-panel"') && html.includes('id="howto-toggle"'));
  check("Онбординг: подсказка first-tip", html.includes('id="first-tip"') && app.includes("showFirstTip"));
  check("Онбординг: initHowtoPanel", app.includes("function initHowtoPanel"));
}
